...
function heapifyDown(arr, n, i, stepList) {
  ...
  if (largest !== i) {
    stepList.push({
      ...
      swapping: [i, largest],
      type: 'swap',
      message: `Swap needed: ${arr[i]} < ${arr[largest]}. Swapping index ${i} with index ${largest}.`,
    });
    ...
    stepList.push({
      ...
      swapping: [],
      type: 'swapped',
      message: `Swapped! Index ${i} is now ${arr[i]}, index ${largest} is now ${arr[largest]}. Continuing down…`,
    });

    heapifyDown(arr, n, largest, stepList);
  } else {
    stepList.push({
      array: [...arr],
      highlighted: [i],
      swapping: [],
      type: 'no-swap',
      message: `No swap needed. ${arr[i]} is already the maximum at index ${i}.`,
    });
  }
}
...
function generateSteps(inputArray) {
  ...
  stepList.push({
    ...
    swapping: [],
    type: 'initial',
    message: `Starting max-heap construction on [${arr.join(', ')}]. Processing from last non-leaf (index ${Math.floor(n / 2) - 1}) up to root.`,
  });
  ...
  stepList.push({
    ...
    swapping: [],
    type: 'complete',
    message: `Max-heap complete! Final array: [${arr.join(', ')}]. Every parent is greater than its children.`,
  });
